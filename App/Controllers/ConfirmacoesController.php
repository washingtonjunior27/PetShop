...
class ConfirmacoesController
{
    ...
    public function PagoAgend()
    {
        if ($_SERVER['REQUEST_METHOD'] === "POST") {
            ...
            $this->agendamentosRepository->UpdateStatusAgend("Em atendimento", $this->agendamentos->getId_agend());

            $_SESSION['sucesso'] = "Confirmado pagamento de agendamento com sucesso!";
            header("location: " . BASE_URL . "/confirmacoes");
            exit;
        } else {
            header("location: " . BASE_URL . "/confirmacoes");
